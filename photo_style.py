...
def contrast_loss(const_layer, var_layer, weight):
    return tf.reduce_mean(tf.squared_difference(const_layer, var_layer)) * weight
...
def print_loss(args, loss_content, loss_styles_list, loss_tv, loss_affine, loss_contrast, overall_loss, output_image):
    ...
    if iter_count % args.print_iter == 0:
        ...
        print('\tContrast loss: {}'.format(loss_contrast))
...
def stylize(args, Matting):
    ...
    sobel_image = utils.sobel(content_image)
    sobel_image = rgb2bgr(sobel_image)
    sobel_image = sobel_image.reshape((1, content_height, content_width, 3)).astype(np.float32)
    ...
    with tf.name_scope("constant"):
        ...
        vgg_const.build(tf.constant(sobel_image), clear_data=False)
        contrast_fv = sess.run(vgg_const.conv4_2)
        contrast_layer_const = tf.constant(contrast_fv)
    ...
    contrast_layer_var = vgg_var.conv4_2
    ...
    # Contrast loss
    loss_contrast = contrast_loss(contrast_layer_const, contrast_layer_var, float(args.contrast_weight))
    ...
    if args.lbfgs:
        if not Matting:
            overall_loss = loss_content + loss_tv + loss_style + loss_contrast
        else:
            overall_loss = loss_content + loss_style + loss_tv + loss_affine + loss_contrast
        ...
        optimizer.minimize(sess, fetches=[loss_content, loss_styles_list, loss_tv, loss_affine, loss_contrast, overall_loss, input_image_plus], loss_callback=print_loss_partial)
        ...
        VGGNetLoss = loss_content + loss_tv + loss_style + loss_contrast
        ...
        for i in xrange(1, args.max_iter):
            _, loss_content_, loss_styles_list_, loss_tv_, loss_affine_, loss_contrast_, overall_loss_, output_image_ = sess.run([
                train_op, loss_content, loss_styles_list, loss_tv, loss_affine, loss_contrast, VGGNetLoss, input_image_plus
            ])
